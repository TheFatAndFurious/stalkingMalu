...
export const parseTopic = (topic: string): Option.Option<ParsedTopic> => {
  ...
  switch (rest.join('/')) {
    case 'telemetry':
      return Option.some({ deviceId, kind: 'telemetry' });
    case 'telemetry/backfill':
      return Option.some({ deviceId, kind: 'telemetry-backfill' });
    case 'status':
      return Option.some({ deviceId, kind: 'status' });
    case 'config':
      return Option.some({ deviceId, kind: 'config' });
    default:
      return Option.none();
  }
};
